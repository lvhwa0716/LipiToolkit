...
public class CanvasClass extends View implements OnTouchListener{
	...
	public void addStroke(Stroke stroke) {
			...
			for (int s = 0; s < _strokes.size(); s++)
				...
				List<LipitkResult> results = _recognizer.recognize(_recognitionStrokes);
						...
						character=new String[results.size()];
						for(int i=0;i<character.length;i++){
							character[i] = _recognizer.getSymbolName(results.get(i).Id, configFileDirectory);
						}
						
						 StrokeResultCount=results.size();
